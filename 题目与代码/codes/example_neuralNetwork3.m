%,改过第二问的w,改过第四问 有early stopping,改过第五问的
tic
load digits.mat
[n,d] = size(X);
nLabels = max(y);
...
stepSize =2e-3;
J=[];
funObj = @(w,i)MLPclassificationLoss5(w,X(i,:),yExpanded(i,:),nHidden,nLabels);
iter = 1;
if mod(iter-1,round(maxIter/20)) == 0
   yhat = MLPclassificationPredict1(w,Xvalid,nHidden,nLabels);
   A= sum(yhat~=yvalid)/t;
